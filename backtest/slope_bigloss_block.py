#!/usr/bin/env python3
"""
Slope strategy — BIG-LOSS-ABOVE-SMA all-directions block (per user 2026-07-11):
  After a loss >= bigLossPts taken while close>SMA, block ALL new entries (both dirs)
  for bigLossCool bars. (Currently only same-dir is blocked; opposite trades slip through.)
Base = validated: noTHRD entries + VWAP proximity ±15.
Sweep bigLossPts {50,60} x bigLossCool {20,30}. Per-year, 0 slip + 1pt.
"""
import csv, sys
...
if __name__=="__main__":
    ...
    for slip in [0.0,1.0]:
        ...
        print(f"{'baseline':>22} | {pnl:>7} {n:>6} {pf:>5} {win:>5} | {pos}/6 | {yl}")
        for lp in [50,60]:
            for cd in [20,30]:
                trs=bt.run(bars, dict(baseP, bigLossPts=float(lp), bigLossCool=int(cd)))
                pnl,n,pf,win,yl,pos=summ(trs,slip,years)
                print(f"{('loss>=%d block%d'%(lp,cd)):>22} | {pnl:>7} {n:>6} {pf:>5} {win:>5} | {pos}/6 | {yl}")
        print()
