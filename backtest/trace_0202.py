...
RE = 0.7
CMIN, CADV, COOL = 30, 40.0, 5   # confirmMins now 30, cancelCool 5
...
for i in range(n):
    ...
    if st == 0:
        ...
        if dN is not None:
            ...
            blk = ""
            if cd != 0:
                blk += " [%s-cooldown blocks re-latch, %d bars left]" % ("UP" if cd > 0 else "DN", COOL-(i-cb))
            if lk != 0:
                blk += " [%s-locked]" % ("UP" if lk > 0 else "DN")
            ev = "flat" + blk
    else:
        ...
        if canc:
            ev = "CANCEL %s age=%d adv=%.0f -> arm %s-cooldown %d bars" % ("UP" if sd > 0 else "DN", age, adv, "UP" if sd > 0 else "DN", COOL)
    ...
    if t.strftime('%d-%b') == '10-Jul' and 1*60+55 <= t.hour*60+t.minute <= 2*60+10:
        print("%s c=%.0f H=%.0f L=%.0f | %s" % (L(i), c, h, l, ev))
