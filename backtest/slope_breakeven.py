#!/usr/bin/env python3
"""
Slope strategy — BREAK-EVEN stop (per user 2026-07-11):
  Philosophy: only lose on an absolute mistake (wrong direction from the start); once a
  trade reaches a reasonable favorable position, never let it become a loss.
  Mechanic: once peak favorable >= beArm, move stop to entry+beOffset (BE). Keep 50 SL
  (catches the wrong-direction mistake) and the +30 trail (locks bigger winners).
Base = validated: noTHRD entries + VWAP proximity ±15.
Sweep beArm {10,15,20} x beOffset {0,1}. Per-year, 0 slip + 1pt. Show exit-reason mix.
"""
import csv, sys
...
if __name__=="__main__":
    ...
    print(f"baseline exit mix: {dict(Counter(t[4] for t in base_trs))}\n")
    for slip in [0.0,1.0]:
        print(f"########## slippage {slip}/trade ##########")
        pnl,n,pf,win,yl,pos=summ(base_trs,slip,years)
        print(f"{'baseline (no BE)':>22} | {pnl:>7} {n:>5} {pf:>5} {win:>5} | {pos}/6 | {yl}")
        for arm in [10,15,20]:
            for off in [0,1]:
                trs=bt.run(bars, dict(baseP, beArm=float(arm), beOffset=float(off)))
                pnl,n,pf,win,yl,pos=summ(trs,slip,years)
                print(f"{('BE arm%d off%d'%(arm,off)):>22} | {pnl:>7} {n:>5} {pf:>5} {win:>5} | {pos}/6 | {yl}")
        print()
    # exit mix for the recommended arm15/off1
    trs=bt.run(bars, dict(baseP, beArm=15.0, beOffset=1.0))
    print(f"arm15/off1 exit mix: {dict(Counter(t[4] for t in trs))}")
    # how many SL losses did BE convert? compare SL count
    print(f"  baseline SL count: {sum(1 for t in base_trs if t[4]=='SL')}, arm15 SL count: {sum(1 for t in trs if t[4]=='SL')}, BE count: {sum(1 for t in trs if t[4]=='BE')}")
