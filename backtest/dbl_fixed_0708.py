...
ext = rows[di][3]
movedaway = False
print('SHORT dot 09:53, time gate removed + not-new-extreme fix')
for k in range(0, 40):
    t, o, h, l, c = rows[di + k]
    pe = ext
    ext = min(ext, l)
    ...
    retestGap = l - ext
    dblexit = proven and movedaway and (not newExt) and 0 <= retestGap <= DBTOL
    tag = 'DBLEXIT' if dblexit else ''
    print('age %2d  %s  L=%.2f ext=%.2f fav=%.0f proven=%s away=%s newExt=%s gap=%.2f  %s' % (
        k, t.strftime('%H:%M'), l, ext, favtrav, 'Y' if proven else 'n', 'Y' if movedaway else 'n', 'Y' if newExt else 'n', retestGap, tag))
    if dblexit:
        break
