...
def run(bars):
    ...
    for bi in range(n):
        ...
        # 1) EXIT breach trade on opposite slope signal
        if inTrade and tradeDir==-1 and oppLong:
            trades.append((-1, entry, c, entry-c)); inTrade=False; tradeDir=0; entry=None
        elif inTrade and tradeDir==1 and oppShort:
            trades.append((1, entry, c, c-entry)); inTrade=False; tradeDir=0; entry=None
        ...
        # 2) ENTER breach trade exactly on the Nth bar
        ...
        if shortDue and tradeDir!=-1:
            ...
            tradeDir=-1; entry=c; inTrade=True
...
if __name__=="__main__":
    ...
    trs = run(bars)
    st=stats(trs)
    print("=== BREACH-FADE 5yr (smoothSR on, breachBars=5, no hard SL, 1pt slip) ===")
