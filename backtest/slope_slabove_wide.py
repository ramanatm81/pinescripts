#!/usr/bin/env python3
"""
Slope strategy — widen slAboveSma (SL when close>SMA) from 50 (per user 2026-07-11).
Base = validated: noTHRD entries + VWAP proximity ±15. Sweep slAboveSma {50,100,150,200}.
Per-year, 0 slip + 1pt. Show SL-loss stats.
"""
import csv, sys
...
KSIGMA=2.0
BASE = dict(slopeEntry=2.5, slBelowSma=30.0, tpPts=50.0, tpMult=3.0,
...
def summ(trs, slip, years):
    ...
    slloss=round(sum(t[3] for t in trs if t[4]=='SL'))
    yl=[]; pos=0
    for y in years:
        yy=round(sum(t[3]-slip for t in trs if t[6].year==y))
        if yy>0: pos+=1
        yl.append(yy)
    return (round(sum(p)), n, round(gp/gl,2) if gl>0 else 999, round(100*w/n,1), yl, pos, slloss)
...
if __name__=="__main__":
    ...
    for slip in [0.0,1.0]:
        ...
        print(f"{'slAboveSma':>12} | {'pnl':>7} {'n':>5} {'pf':>5} {'win':>5} {'SLloss':>8} | +yr | per-year")
        for sla in [50,100,150,200]:
            trs=bt.run(bars, dict(baseP, slAboveSma=float(sla)))
            pnl,n,pf,win,yl,pos,slloss=summ(trs,slip,years)
            star=" <- current" if sla==50 else (" <- your 200" if sla==200 else "")
            print(f"{sla:>12} | {pnl:>7} {n:>5} {pf:>5} {win:>5} {slloss:>8} | {pos}/6 | {yl}{star}")
        print()
