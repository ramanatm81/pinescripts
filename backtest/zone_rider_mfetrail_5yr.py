...
def run(bars, piv, useMFEtrail, freshBreach):
    n=len(bars)
    ...
    for bi in range(n):
        ...
        if inTrade:
            ...
            if not exited and useMFEtrail and mfe>=MFE_TRIG:
                if tradeDir==1 and l<=peak-MFE_TRAIL: trades.append((1, (peak-MFE_TRAIL)-entry)); exited=True
                elif tradeDir==-1 and h>=peak+MFE_TRAIL: trades.append((-1, entry-(peak+MFE_TRAIL))); exited=True
            ...
            if exited:
                ...
                if freshBreach:   # kill the current breach so it can't re-enter until it re-forms
                    upB=0; dnB=0
...
if __name__=="__main__":
    ...
    configs=[("BASELINE (Denver, no MFE-trail)", False, False),
             ("+ MFE100/trail60", True, False),
             ("+ MFE100/trail60 + fresh-breach", True, True)]
    print(f"{'config':38} | {'pnl':>7} {'n':>5} {'pf':>5} {'win':>5} {'mdd':>6} | +yrs | per-year")
    for name,mt,fb in configs:
        s=stat(run(bars,piv,mt,fb))
        ys=[stat(run(yr_bars[yr],yr_piv[yr],mt,fb))['pnl'] for yr in years]
        pos=sum(1 for y in ys if y>0)
        print(f"{name:38} | {s['pnl']:>7} {s['n']:>5} {s['pf']:>5} {s['win']:>5} {s['mdd']:>6} | {pos}/6 | {[round(y) for y in ys]}")
